...
const ScheduleView: React.FC<ScheduleViewProps> = ({ schedule, isLoading, error, onUpdateSession, onToggleComplete, progress, onSaveSchedule }) => {
  ...
  return (
    <div className="bg-white p-4 sm:p-6 rounded-lg shadow-md schedule-container">
        <style>{`
          @media print {
            .schedule-container {
        ...
        {schedule && schedule.length > 0 && (
          <div className="print-only w-full">
            <div className="mb-6">
                <h1 className="text-2xl font-bold text-black mb-1">GCSE Revision Schedule</h1>
                <p className="text-sm text-gray-600">Generated on {new Date().toLocaleDateString()}</p>
            </div>
            <table className="w-full text-left border-collapse border border-gray-400">
              <thead>
                <tr className="bg-gray-100">
                  <th className="py-2 px-3 border border-gray-400 font-bold text-black w-1/5 text-sm">Time</th>
                  <th className="py-2 px-3 border border-gray-400 font-bold text-black w-3/5 text-sm">Activity</th>
                  <th className="py-2 px-3 border border-gray-400 font-bold text-black w-1/5 text-sm">Subject</th>
                </tr>
              </thead>
              <tbody>
                {schedule.map((session) => (
                  <tr key={session.id} className="border-b border-gray-400">
                    <td className="py-2 px-3 border border-gray-400 text-black whitespace-nowrap align-top text-sm font-medium">
                      {formatTime(session.startTime)} - {formatTime(session.endTime)}
                    </td>
                    <td className="py-2 px-3 border border-gray-400 text-black align-top text-sm">
                      {session.activity}
                    </td>
                    <td className="py-2 px-3 border border-gray-400 text-black align-top text-sm">
                      {session.subject || '-'}
